...
struct AttentionKernel {
  ...
  CUTLASS_DEVICE static void iterative_softmax(
    ...
    constexpr float kLog2e = 1.4426950408889634074; // log_2(e)
  ...
  // --- Mitigation 2: Noise Injection  ---
    ...
    if (noise_scale > 0.0f) {
      LambdaIterator::iterateRows(
          ...
          [&](int accum_m, int accum_n, int idx) {
            if (accum_n < max_col) {
              // CORRECTED: Generate two independent uniform random numbers for Box-Muller.
              // This is critical for generating a correct Gaussian distribution.

              // First uniform random number (u1)
              uint32_t seed1 = (thread_id * 71) ^ (idx * 113) ^ (accum_n * 3);
              seed1 ^= seed1 << 13;
              seed1 ^= seed1 >> 17;
              seed1 ^= seed1 << 5;
              accum_t u1 = (static_cast<accum_t>(seed1) + 1.0f) / 4294967297.0f;

              // Second uniform random number (u2)
              uint32_t seed2 = (thread_id * 19) ^ (idx * 251) ^ (accum_m * 7);
              seed2 ^= seed2 << 13;
              seed2 ^= seed2 >> 17;
              seed2 ^= seed2 << 5;
              accum_t u2 = (static_cast<accum_t>(seed2) + 1.0f) / 4294967297.0f;

              // Box-Muller transform to get a standard normal random number
              constexpr float kPi = 3.141592653589793f;
              accum_t noise = noise_scale *
                              sqrtf(-2.0f * logf(u1)) * cosf(2.0f * kPi * u2);
              //Clip noise for stability (creates a Truncated Normal distribution) ---
              constexpr float max_noise = 0.05f;
              noise = cutlass::fast_max(-max_noise, cutlass::fast_min(max_noise, noise));
              frag[idx] += noise;
            }
          },
          [&](int accum_m) {});
